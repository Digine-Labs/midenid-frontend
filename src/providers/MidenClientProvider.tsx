...
import { AccountId, type Account, type MidenClient } from '@miden-sdk/miden-sdk';
...
// Miden 0.15 derives storage slot names from the package name and dropped the
// `miden_` prefix 0.14 used, so this is no longer the assembly contract's
// `naming::domain_to_owner`. Regenerated by the contracts repo; see
// src/shared/generated/registry.ts.
import { DOMAIN_TO_OWNER_SLOT } from '@/shared/generated/registry';
  ...
  const getDomainOwner = useCallback(
    async (domain: string): Promise<AccountId | null> => {
      ...
      return mutexRef.current.runExclusive(async () => {
        ...
        // No felt reversal against the Rust registry. The assembly contract loaded the
        // domain through `mem_loadw_be` before writing the map entry, which flipped the
        // word into MASM stack order and forced callers to mirror that. The Rust
        // contract uses the domain word exactly as the note supplied it, so the key is
        // the encoded word as-is.
        const ownerWord = account.storage().getMapItem(DOMAIN_TO_OWNER_SLOT, domainWord);
